...
/**
 * This is the model class for table "rutour".
 *
 * @property int $id
 ...
 */
class Rutour extends ActiveRecord
{
	...
	public function rules()
	{
		return [
			...
			[['status', 'name', 'order_index'], 'required'],
			[['status', 'created_at', 'updated_at','bottom_menu','bottom_menu_index', 'order_index'], 'integer'],
	...
	public function attributeLabels()
	{
		return [
			...
			'gallery' => 'Галерея',
			'status' => 'Статус',
			'order_index' => 'Порядковый номер на главной стр.',
	...
	public function upload(){
		if($this->validate()){
			...
			//die();
			$this->attachImage($path, true);//метод сам сгенерирует уник.имя и привяжет её к модели в БД
			@unlink($path);
			return true;
		}else{
			return false;
